...
3. 최악의 경우) n-k번의 루프를 돌며 모든 경우에서 min_num 값을 새로 구해줘야 하는 경우가 존재한다. 이 경우에는 k 값만큼의 범위에서 min값을 구하기 때문에 min값을 구하는 수행시간은 k가 나올 것이고 k(n-k) = nk-k^2 수행시간이 걸릴 것이다. 
4. 평균적으로 O(n) 수행시간이 걸릴 것이다.
"""
